<x-app-layout>
    ...
    @php
        $filtrosAtivos = collect([
            ...
            'Prontidao S-1040' => match ($filtros['prontidao']) {
                'pronta' => 'Pronta',
    ...
    <section class="space-y-6">
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            <div class="stat-card">
                <p class="text-sm text-slate-400">Total de funcoes</p>
                <p class="mt-3 text-3xl font-semibold text-white">{{ number_format($resumo['total'], 0, ',', '.') }}</p>
                <p class="mt-2 text-sm text-cyan-300">Base complementar do orgao</p>
            </div>
            <a href="{{ route('funcoes.index', ['status' => 'ativos']) }}" class="stat-card block transition hover:border-emerald-400/40 hover:bg-emerald-500/5 focus:outline-none focus:ring-2 focus:ring-emerald-400/50">
            ...
            <a href="{{ route('funcoes.index', ['status' => 'inativos']) }}" class="stat-card block transition hover:border-amber-400/40 hover:bg-amber-500/5 focus:outline-none focus:ring-2 focus:ring-amber-400/50">
            ...
            <a href="{{ route('funcoes.index', ['prontidao' => 'pronta']) }}" class="stat-card block transition hover:border-emerald-400/40 hover:bg-emerald-500/5 focus:outline-none focus:ring-2 focus:ring-emerald-400/50">
                <p class="text-sm text-slate-400">Prontas S-1040</p>
                <p class="mt-3 text-3xl font-semibold text-white">{{ number_format($resumo['s1040_prontas'], 0, ',', '.') }}</p>
                <p class="mt-2 text-sm text-emerald-300">Ativas com codigo eSocial</p>
            </a>
            <a href="{{ route('funcoes.index', ['prontidao' => 'pendente']) }}" class="stat-card block transition hover:border-amber-400/40 hover:bg-amber-500/5 focus:outline-none focus:ring-2 focus:ring-amber-400/50">
                <p class="text-sm text-slate-400">Pendencias S-1040</p>
                <p class="mt-3 text-3xl font-semibold text-white">{{ number_format($resumo['s1040_pendentes'], 0, ',', '.') }}</p>
        ...
        <div class="grid gap-4 md:grid-cols-2">
            <div class="stat-card">
                ...
                <p class="mt-2 text-sm text-cyan-300">Base para S-1040</p>
            </div>
            <div class="stat-card">
                ...
                <p class="mt-2 text-sm text-amber-300">Pendencia funcional</p>
            </div>
        </div>
        ...
        <div class="grid gap-6 xl:grid-cols-[1.8fr_1fr]">
            <div class="panel-surface rounded-3xl p-6">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        ...
                        <h2 class="mt-2 text-2xl font-semibold text-white">Funcoes estruturadas para RH e eSocial</h2>
                        <p class="mt-3 max-w-2xl text-sm leading-6 text-slate-300">
                            Este modulo organiza funcoes complementares que detalham responsabilidades e ajudam a manter a classificacao funcional do sistema.
                        </p>
                    </div>

                    <div class="flex w-full max-w-3xl flex-col gap-3 lg:items-end">
                        <a href="{{ route('funcoes.create') }}" class="btn btn-info">Nova funcao</a>
                        <form method="GET" action="{{ route('funcoes.index') }}" class="flex w-full flex-col gap-3 xl:flex-row xl:flex-wrap xl:items-end">
                            ...
                            <label class="form-control w-full xl:w-52">
                                ...
                                <select name="prontidao" class="select select-bordered w-full border-white/10 bg-slate-950/50 text-sm text-white">
                                    ...
                                    <option value="pronta" @selected($filtros['prontidao'] === 'pronta')>Pronta S-1040</option>
                            ...
                            <div class="flex w-full flex-col gap-3 sm:flex-row xl:w-auto xl:flex-none">
                                ...
                                <a href="{{ route('funcoes.index') }}" class="btn btn-ghost w-full sm:w-auto">Limpar</a>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                @if ($filtrosAtivos->isNotEmpty())
                    <div class="mt-6 flex flex-col gap-3 rounded-2xl border border-cyan-400/20 bg-cyan-500/10 p-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <a href="{{ route('funcoes.index') }}" class="btn btn-ghost btn-sm">Limpar filtros</a>
                    </div>
                @endif
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @forelse ($funcoes as $funcao)
                                <tr>
                                    <td class="font-medium text-white">{{ $funcao->codigo }}</td>
                                    <td>{{ $funcao->nome }}</td>
                                    <td>{{ $funcao->codigo_esocial ?? 'Nao informado' }}</td>
                                    <td>{{ number_format($funcao->servidores_count, 0, ',', '.') }}</td>
                                    <td>
                                        <span class="badge badge-outline {{ $funcao->ativo ? 'badge-success' : 'badge-warning' }}">
                                            {{ $funcao->ativo ? 'Ativa' : 'Inativa' }}
                                        </span>
                                    </td>
                                    <td class="text-right">
                                        <a href="{{ route('funcoes.edit', $funcao) }}" class="btn btn-ghost btn-sm">Editar</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="6" class="py-10 text-center text-sm text-slate-400">
                                        Nenhuma funcao encontrada com os filtros atuais.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            <div class="space-y-6">
                <div class="panel-surface rounded-3xl p-6">
                    ...
                    <h3 class="mt-3 text-xl font-semibold text-white">Base para S-1040</h3>
                    <ul class="mt-4 space-y-3 text-sm leading-6 text-slate-300">
                        <li>A funcao complementa a leitura do exercicio real do trabalhador.</li>
                        <li>O codigo interno organiza a operacao; o codigo eSocial prepara o espelhamento governamental.</li>
                        <li>Funcoes prontas nesta etapa sao ativas e possuem codigo eSocial informado.</li>
                        <li>Esse cadastro melhora a qualidade de manutencao do vinculo funcional.</li>
                    </ul>
                </div>
            </div>
        </div>
