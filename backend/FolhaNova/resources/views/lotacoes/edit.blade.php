            ...
            <div class="panel-surface rounded-3xl p-6">
                <div class="flex flex-col gap-3 border-b border-white/10 pb-5 md:flex-row md:items-start md:justify-between">
                    <div>
                        ...
                        <h2 class="text-2xl font-semibold text-white">Atualizacao de lotacao</h2>
                        <p class="max-w-3xl text-sm leading-6 text-slate-300">
                            Ajuste os dados estruturais da lotacao preservando o cadastro usado pelos vinculos existentes.
                        </p>
                    </div>

                    <a href="{{ route('lotacoes.index') }}" class="btn btn-ghost">Voltar para lista</a>
                </div>
                ...
                <form method="POST" action="{{ route('lotacoes.update', $lotacao) }}" class="mt-6 space-y-8">
                    @csrf
                    @method('PUT')
            ...
            <div class="space-y-6">
                <div class="panel-surface rounded-3xl p-6">
                    ...
                    <ul class="mt-4 space-y-3 text-sm leading-6 text-slate-300">
                        <li>Codigo: {{ $lotacao->codigo }}</li>
                        <li>Servidores vinculados: {{ $lotacao->servidores()->count() }}</li>
                        <li>Status: {{ $lotacao->ativa ? 'Ativa' : 'Inativa' }}</li>
                    </ul>
                </div>
            </div>
        </div>
